...
const getTags = async (page) => {
   ...
   try {
      const res = await axios.get(route('tag.data') + '?' + searchParams.toString())
...
const handleDelete = async(id) => {
  HelperService.confirmDelete(
    async() => await axios.delete(route('tag.destroy', id)),
    async () => {
...
<template>
   <AdminLayout>
      <PageBreadcrumb :page-list="[
         { label: 'Tag Management', href: '', currPage: true }
      ]" />
      ...
      <ComponentCard title="Data Tag">
         ...
         <Table :data="lists" :field="fields" @page-change="handlePageChange" @sort-change="handleSortData">
            ...
            <template #action="{ row }">
               <div class="flex gap-2">
                  <Link :href="route('tag.edit', row.id)">
