...
const getTypes = async (page) => {
   ...
   try {
      const res = await axios.get(route('content-type.data') + '?' + searchParams.toString())
...
const handleDelete = async(id) => {
  HelperService.confirmDelete(
    async() => await axios.delete(route('content-type.destroy', id)),
    async () => {
...
<template>
   <AdminLayout>
      <PageBreadcrumb :page-list="[
         { label: 'Content Type Management', href: '', currPage: true }
      ]" />

      <ComponentCard title="Data Content Type">
         <template #searchbar>
            <div class="flex items-center gap-2">
               <TextInput
                  @keyup.enter="getTypes"
                  class="w-72"
                  v-model="searchText"
         ...
         <template #button>
            <Link :href="route('content-type.create')">
         ...
         <Table :data="lists" :field="fields" @page-change="handlePageChange" @sort-change="handleSortData">
            ...
            <template #description="{ row }">
               <div class="flex gap-2">
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400"><strong>ID:</strong> {{ row.description['id'].slice(0, 150) }}</p>
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400"><strong>EN:</strong> {{ row.description['en'].slice(0, 150) }}</p>
               </div>
            </template>
            <template #action="{ row }">
               <div class="flex gap-2">
                  <Link :href="route('content-type.edit', row.id)">
