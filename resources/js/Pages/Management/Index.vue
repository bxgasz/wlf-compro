...
const fields = [
   ...
   { "key": "active", "label": "Status", "sort": true },
   { "key": "action", "label": "Action" },
]

const lists = ref(props.managements)
...
const getManagements = async (page) => {
   ...
   try {
      const res = await axios.get(route('management.data') + '?' + searchParams.toString())
...
const handleDelete = async(id) => {
  HelperService.confirmDelete(
    async() => await axios.delete(route('management.destroy', id)),
    async () => {
...
const toggleStatus = async(event, item) => {
  event.preventDefault();
    ...
    Swal.fire({
        icon: 'question',
        title: 'Warning',
        text: 'Are you sure want to change status of this '+ `${item.title.en} | ${item.title.id}` +' ?',
        ...
        if(isConfirmed) {
         try {
            await axios.post(route('management.status', item.id)).then(() => {
...
<template>
   <AdminLayout>
      <PageBreadcrumb :page-list="[
         { label: 'Management', href: '', currPage: true }
      ]" />

      <ComponentCard title="Data Management">
         <template #searchbar>
            <div class="flex items-center gap-2">
               <TextInput
                  @keyup.enter="getManagements"
                  class="w-72"
                  v-model="searchText"
         ...
         <Table :data="lists" :field="fields" :order="order" @page-change="handlePageChange" @sort-change="handleSortData">
            ...
            <template #action="{ row }">
               <div class="flex gap-2">
                  <Link :href="route('management.edit', row.id)">
