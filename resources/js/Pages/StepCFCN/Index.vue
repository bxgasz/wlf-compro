...
const getSteps = async (page) => {
   ...
   try {
      const res = await axios.get(route('step-cfcn.data') + '?' + searchParams.toString())
...
const handleDelete = async(id) => {
  HelperService.confirmDelete(
    async() => await axios.delete(route('step-cfcn.destroy', id)),
    async () => {
...
<template>
   <AdminLayout>
      <PageBreadcrumb :page-list="[
         { label: 'Step Management', href: '', currPage: true }
      ]" />

      <ComponentCard title="Data Step">
         <template #searchbar>
            <div class="flex items-center gap-2">
               <TextInput
                  @keyup.enter="getSteps"
                  class="w-72"
                  v-model="searchText"
         ...
         <template #button>
            <Link :href="route('step-cfcn.create')">
         ...
         <Table :data="lists" :field="fields" @page-change="handlePageChange" @sort-change="handleSortData">
            ...
            <template #action="{ row }">
               <div class="flex gap-2">
                  <Link :href="route('step-cfcn.edit', row.id)">
