...
const getNewsStories = async (page) => {
   ...
   try {
      const res = await axios.get(route('news-stories.data') + '?' + searchParams.toString())
...
const handleDelete = async(id) => {
   HelperService.confirmDelete(
      async() => await axios.delete(route('news-stories.destroy', id)),
      async() => {
...
<template>
   <AdminLayout>
      ...
      <ComponentCard title="Data News Stories">
         ...
         <template #button>
            <Link :href="route('news-stories.create')">
         ...
         <Table :data="lists" :field="fields" :order="order" @page-change="handlePageChange" @sort-change="handleSortData">
            ...
            <template #action="{ row }">
               <div class="flex gap-2">
                  <Link :href="route('news-stories.edit', row.id)">
