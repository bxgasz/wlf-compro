...
const getBanners = async (page) => {
   ...
   try {
      const res = await axios.get(route('banner.data') + '?' + searchParams.toString())
...
const toggleStatus = async(event, item) => {
  event.preventDefault();
    ...
    Swal.fire({
        ...
        if(isConfirmed) {
         try {
            await axios.post(route('banner.status', item.id)).then(() => {
...
const handleDelete = async(id) => {
  HelperService.confirmDelete(
    async() => await axios.delete(route('banner.destroy', id)),
    async () => {
...
<template>
   <AdminLayout>
      <PageBreadcrumb :page-list="[
         { label: 'Banner Management', href: '', currPage: true }
      ]" />

      <ComponentCard title="Data Banner">
         <template #searchbar>
         ...
         <Table :data="lists" :field="fields" :order="order" @page-change="handlePageChange" @sort-change="handleSortData">
            ...
            <template #action="{ row }">
               <div class="flex gap-2">
                  <Link :href="route('banner.edit', row.id)">
